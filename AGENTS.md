...
- `chrome-extension/` — MV3 form-autofill/copilot extension (`webform-copilot`), fully built. Next.js 14 static export (`output: "export"`, Pages Router) + esbuild for the background SW; `scripts/postbuild.mjs` renames `_`-prefixed files Next emits (`_next/` → `next-assets/`) since Chrome rejects reserved names. Build: `npm run build` → load `out/` unpacked. Verify with `npx tsc --noEmit` (no eslint dep). One side panel with 3 views — **login** (`POST /api/admin/login`, tokens in `chrome.storage.session`, auto-refresh via `/api/admin/refresh`), **main** (capture/upload + mock doc-type selector `Registrasi Pasien`/`Inventory` → `POST /api/admin/ocr/extract`), **settings** (base URL in `chrome.storage.local` + logout). Fetches directly from the sidepanel (CORS), no content scripts. `lib/auth.ts`, `lib/ocr.ts`, `lib/settings.ts`, `lib/types.ts`.
