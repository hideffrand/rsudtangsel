...
- **OCR proxy**: OCR is a separate microservice (`ocr-service/`, FastAPI). All apps hit the Go server - `POST /api/ocr/extract` (public) and `/api/admin/ocr/extract` (JWT-protected) - and the Go server proxies to `OCR_SERVICE_URL` (default `http://localhost:8000`, appends `/ocr/extract`). Response envelope is kept; upstream errors return 502. Run `docker compose up -d ocr` to start it.
