"""
Document Parser - RSU Tangsel Care (TrOCR Edition)
Memproses teks mentah hasil ekstraksi Microsoft TrOCR menjadi field terstruktur (NIK, Nama Lengkap, Umur, JK, dll).
"""
...
def parse_registrasi_pasien(raw_text: str, blocks: List[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    ...
    if nama_match:
        ...
        # Fallback baris pertama jika berbentuk nama
        if lines:
            first_line = lines[0]
            # Bersihkan label jika ada
    ...
    if umur_match:
        ...
        # Cari angka yang diikuti kata 'tahun' / 'thn'
    ...
    fields.append({
        "key": "Umur",
        "value": umur_val,
        "confidence": 92.0 if umur_val else 45.0,
        "is_required": False,
    })

    # 4. Jenis Kelamin (Laki-laki / Perempuan / L / P)
    jk_val = ""
    ...
    fields.append({
        "key": "Jenis Kelamin",
        "value": jk_val,
        "confidence": 94.0 if jk_val else 45.0,
        "is_required": False,
    })
    ...
    fields.append({
        "key": "No. Telepon",
        "value": telp_val,
        "confidence": 92.0 if telp_val else 40.0,
        "is_required": False,
    })
