 ...
 * 2. Review hasil ekstraksi (field confidence rendah di-highlight warna kuning/merah)
 * 3. Approve/Reject sebelum sync ke SIMRS
 */
...
interface OcrDoc {
  id: string;
  docType: "KTP" | "BPJS" | "Surat Rujukan" | "Resep Dokter";
  patientName: string;
  uploadDate: string;
  ...
  const handleSimulatedUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTimeout(() => {
      const newDoc: OcrDoc = {
        ...
        docType: "KTP",
        patientName: "Dokumen Baru Upload",
        uploadDate: new Date().toISOString().slice(0, 16).replace("T", " "),
        status: "Pending Review",
        confidenceScore: 78,
        extractedFields: [
          { key: "NIK", value: "3674998811220003", confidence: 92 },
          { key: "Nama", value: "Dokumen Baru Upload", confidence: 85 },
          { key: "Alamat", value: "Jl. BSD Grand Boulevard", confidence: 58 }, // Highlight low confidence
        ],
      };
      setDocs([newDoc, ...docs]);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Verifikasi &amp; koreksi hasil ekstraksi AI pada dokumen (KTP, BPJS, Rujukan) sebelum disimpan ke SIMRS.
          </p>
        </div>

        {/* Upload Button */}
        <label className="px-4 py-2 text-sm font-semibold text-white bg-emerald-600 hover:bg-emerald-500 rounded-lg cursor-pointer transition-colors shadow-xs flex items-center gap-2 w-fit">
          {isUploading ? (
            <>
            ...
            <>
              ...
              <input type="file" accept="image/*,.pdf" onChange={handleSimulatedUpload} className="hidden" />
            </>
          )}
        </label>
      </div>
